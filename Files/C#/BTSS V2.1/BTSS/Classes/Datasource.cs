...
namespace Biztech.Classes
{
    abstract class Datasource
    {
        ...
        private Global.DataProvider dataProvider;
                
        protected internal Datasource(string connectionString, Global.DataProvider provider)
        {
            try 
            {
                dataProvider = provider;

                if (dataProvider == Global.DataProvider.SQL)
        ...
        protected internal void ExecuteScalar(string sql)
        {
            ...
            else if (dataProvider == Global.DataProvider.OLEDB)
            {
        ...
        protected internal string  GetNextCode(string tableName,string fieldName,string sequence) 
    {
        ...
        protected internal bool IsValueAlreadyExist(string tableName, string fieldName, string originalvalue, string currentValue, Global.Operation operation)
        {
            try
            {
                string sql = "";
                ...
                { 
                    ...
                    else if (operation == Global.Operation.UPDATE)
                    { 
                        sql = "SELECT " +					 
				            " CASE WHEN COUNT("+  fieldName +") > 0 THEN " +
				            "   1" +
